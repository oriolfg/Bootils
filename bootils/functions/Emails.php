...
class Emails
{
    ...
    public function swiftMail ($to, $subject, $body, $values)
    {
        //Default values
        ...
        if (!defined("SMTP_ANTIFLOOD")) {
            define('SMTP_ANTIFLOOD', true);
        }
        if (!defined("SMTP_ANTIFLOOD_MAILS")) {
            define('SMTP_ANTIFLOOD_MAILS', 100);
        }
        if (!defined("SMTP_ANTIFLOOD_PAUSE")) {
            define('SMTP_ANTIFLOOD_PAUSE', 30);
        }
        if (isset($values['sender'])) {
            $from=$values['sender'];
        } else {
            if (!defined("MAIL_SENDER_MAIL")) {
                define('MAIL_SENDER_MAIL', '');
            }
    ...
    public function phpMail ($to, $subject, $body, $values)
    {
        // recipients
        $recipients=array();
        if (is_array($to)) {
