...
if (isset($_bootils) && is_array($_bootils)) {
...
if (!defined("DEFAULT_LANGUAGE_LOCALE")) {
    define('DEFAULT_LANGUAGE_LOCALE', 'en_GB.utf8');
}
...
if (defined("ERROR_REPORTING") && ERROR_REPORTING==true) {
    $tmp=array();
    if (defined("HIDE_ERROR") && HIDE_ERROR==true) {
        $tmp[]='E_ERROR';
    }
    if (defined("HIDE_RECOVERABLE_ERROR") && HIDE_RECOVERABLE_ERROR==true) {
        $tmp[]='E_RECOVERABLE_ERROR';
    }
    if (defined("HIDE_WARNING") && HIDE_WARNING==true) {
        $tmp[]='E_WARNING';
    }
    if (defined("HIDE_PARSE") && HIDE_PARSE==true) {
        $tmp[]='E_PARSE';
    }
    if (defined("HIDE_STRICT") && HIDE_STRICT==true) {
        $tmp[]='E_STRICT';
    }
    if (defined("HIDE_NOTICE") && HIDE_NOTICE==true) {
        $tmp[]='E_NOTICE';
    }
    if (defined("HIDE_DEPRECATED") && HIDE_DEPRECATED==true) {
        $tmp[]='E_DEPRECATED';
    }
    if (defined("HIDE_USER_DEPRECATED") && HIDE_USER_DEPRECATED==true) {
