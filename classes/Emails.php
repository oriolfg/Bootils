...
function phpMailer ($to, $subject, $body, $values, $mailConf)
{
    ...
    if (isset($mailConf->mail_reply_to_name) && $mailConf->mail_reply_to_name != null) {
        $mail->addReplyTo($mailConf->mail_reply_to_mail, $mailConf->mail_reply_to_name);
    } else {
...
function swiftMail ($to, $subject, $body, $values, $mailConf)
{
    ...
    $transport = \Swift_SmtpTransport::newInstance($mailConf->smtp_host, $mailConf->smtp_port)
    ...
    if ($mailConf->smtp_antiflood==true) {
        $mailer->registerPlugin(new \Swift_Plugins_AntiFloodPlugin($mailConf->smtp_antiflood_mails, $mailConf->smtp_antiflood_pause));
    }
    ...
    if (isset($values['plain_charset'])) {
        ...
        $message->addPart(html2txt($body), 'text/plain', $mailConf->mail_charset_html);
    }
